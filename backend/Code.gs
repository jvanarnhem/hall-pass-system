...
function handleCheckOut(params) {
  ...
  const dateOnly = Utilities.formatDate(checkOutTime, Session.getScriptTimeZone(), 'M/d/yyyy');
  ...
  const timeValue = (checkOutTime.getHours() + checkOutTime.getMinutes() / 60 + checkOutTime.getSeconds() / 3600) / 24;
...
function handleCheckIn(params) {
  ...
  for (let i = 1; i < activeData.length; i++) {
    ...
    if (String(activeData[i][1]) === String(passId)) { // Column B = PassID
      passRow = i + 1;
  ...
  const checkOutTimeFormatted = Utilities.formatDate(checkOutTime, Session.getScriptTimeZone(), 'h:mm:ss a');
  const checkInTimeFormatted = Utilities.formatDate(checkInTime, Session.getScriptTimeZone(), 'h:mm:ss a');
  const dateOnly = Utilities.formatDate(checkInTime, Session.getScriptTimeZone(), 'M/d/yyyy');
...
function getAnalytics(days) {
  ...
  const frequentUsers = Object.keys(monthlyCount).map(id => ({
    studentId: id,
    name: monthlyCount[id].name,
    count: monthlyCount[id].count
  })).sort((a, b) => b.count - a.count);

  const dailyMultiple = Object.keys(dailyCount).map(id => ({
    studentId: id,
    name: dailyCount[id].name,
    count: dailyCount[id].count,
    date: now.toISOString()
  })).filter(u => u.count >= 3).sort((a, b) => b.count - a.count);
...
function getRoomList() {
  try {
    ...
    Logger.log('Found ' + sortedRooms.length + ' unique rooms: ' + sortedRooms.join(', '));
...
function getDestinationList() {
  try {
    ...
    Logger.log('Found ' + destinations.length + ' destinations: ' + destinations.join(', '));
...
function importRosterCSV(csvData) {
  try {
    ...
    const startRow = (rows[0][0].toLowerCase().includes('id') || 
                      rows[0][0].toLowerCase().includes('student') ||
                      rows[0][1].toLowerCase().includes('name')) ? 1 : 0;
...
function logActivity(action, studentId, studentName, room, destination) {
  ...
  Logger.log(`${action}: ${studentName} (${studentId}) - ${room} to ${destination}`);
}
  ...
  try {
    ...
    for (let i = 1; i < archiveData.length; i++) {
      ...
      if (checkOutTime >= todayStart && checkOutTime <= todayEnd) {
        ...
        if (row[9]) { // CheckInTime column
          // CheckInTime is stored as a time string, combine with today's date
          const checkInTimeStr = row[9];
          checkInTime = new Date(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate());

          // If row[9] is a Date object (time from 1899)
          if (row[9] instanceof Date) {
            checkInTime.setHours(row[9].getHours(), row[9].getMinutes(), row[9].getSeconds());
          } else {
            // If it's a string, try to parse it
            const timeParts = checkInTimeStr.match(/(\d+):(\d+):(\d+)\s*(AM|PM)/i);
            if (timeParts) {
    ...
    allPasses.sort((a, b) => new Date(b.checkOutTime) - new Date(a.checkOutTime));
